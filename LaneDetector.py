...
class Car:
    def __init__(self) -> None:
        ...
        self.left_tracker = Tracker((0.2, 0.7), 100)
    ...
    def straight(self):
        ...
        if (right_tracker_dir_val == Direction.LEFT.value and left_tracker_dir_val == Direction.RIGHT.value) or \
            (right_tracker_dir_val == Direction.RIGHT.value and left_tracker_dir_val == Direction.LEFT.value):
            self.direction = Direction.STRAIGHT
    ...
    def __call__(self, frame):
        ...
        if turn.value == Direction.LEFT.value or (self.recent_tracker is not None and not self.recent_tracker.is_active):
            # self.recent_tracker = self.right_tracker
            self.direction = turn
            self.recent_tracker = self.right_tracker
        elif turn.value == Direction.RIGHT.value  or (self.recent_tracker is not None and not self.recent_tracker.is_active):
            self.direction = turn
            self.recent_tracker = self.left_tracker
        else:
            self.straight()
            self.recent_tracker = None



        # if self.turn.value == Direction.STRAIGHT.value:
        #     if left_tracker_direction.value * right_tracker_direction.value in [-1, 0]:
        #         self.direction = Direction.STRAIGHT
        #     else:
        ...
        # cv2.circle(mask, (IMAGE_SIZE[0]//2, IMAGE_SIZE[1]-1), 200, (0, 0, 255), 3)
